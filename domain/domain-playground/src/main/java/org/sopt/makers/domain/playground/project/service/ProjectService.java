...
public class ProjectService {
    ...
    public void createProject(
            String name, Long writerId, Integer generation, String category,
            LocalDate startAt, LocalDate endAt, List<String> serviceType,
            Boolean isAvailable, Boolean isFounding, String summary, String detail,
            String logoImage, String thumbnailImage, List<String> images,
            List<ProjectMember> members, List<ProjectLink> links
    ) {
        validateImageCount(images.size());
        Project project = projectRepositoryPort.save(new Project(
                null, writerId, name, generation, category, startAt, endAt,
                serviceType, isAvailable, isFounding, summary, detail,
                logoImage, thumbnailImage, images,
                LocalDateTime.now(), LocalDateTime.now()
        ));
        projectMemberRepositoryPort.saveAll(members.stream()
                .map(m -> new ProjectMember(null, project.id(), m.userId(), m.role(), m.description(), m.isTeamMember()))
                .toList());
        projectLinkRepositoryPort.saveAll(links.stream()
                .map(l -> new ProjectLink(null, project.id(), l.title(), l.url()))
                .toList());
    }

    @Transactional
    public void updateProject(
            Long writerId, Long projectId,
            String name, Integer generation, String category,
            LocalDate startAt, LocalDate endAt, List<String> serviceType,
            Boolean isAvailable, Boolean isFounding, String summary, String detail,
            String logoImage, String thumbnailImage, List<String> images,
            List<ProjectMember> members, List<ProjectLink> links
        ...
        projectRepositoryPort.save(new Project(
                project.id(), project.writerId(),
    ...
    public List<Project> fetchAll(Integer limit, Long cursor, String searchWord, String category,
            Boolean isAvailable, Boolean isFounding, Integer generation) {
        return projectQueryPort.findProjects(limit, cursor, searchWord, category, isAvailable, isFounding, generation);
    }
    ...
    public ProjectDetailResult getProjectDetail(Long projectId) {
        ...
        Map<Long, ProjectUserInfo> userInfoMap = projectUserPort.getProjectUserInfosByIds(userIds).stream()
                .collect(Collectors.toMap(ProjectUserInfo::id, Function.identity()));

        List<ProjectMemberResult> memberResults = projectMembers.stream()
                .map(member -> {
                    ...
                    return new ProjectMemberResult(
                            member.userId(), member.role(), member.description(), member.isTeamMember(),
                            info.name(), info.generations(), info.profileImage(), info.hasProfile()
                    );
                })
    ...
    @Transactional(readOnly = true)
    public int getProjectsCount(String searchWord, String category, Boolean isAvailable,
            Boolean isFounding, Integer generation) {
        return projectQueryPort.countAllProjects(searchWord, category, isAvailable, isFounding, generation);
    }
    ...
    private void updateProjectMembers(Long projectId, List<ProjectMember> requestMembers) {
        ...
        Map<Long, ProjectMember> existingMap = existing.stream()
                .collect(Collectors.toMap(ProjectMember::userId, Function.identity()));
        ...
        List<ProjectMember> toRemove = existing.stream()
                .filter(m -> !requestedUserIds.contains(m.userId()))
                .toList();
        projectMemberRepositoryPort.deleteAll(toRemove);

        List<ProjectMember> toSave = requestMembers.stream()
                .map(m -> {
                    ...
                    return new ProjectMember(null, projectId, m.userId(), m.role(), m.description(), m.isTeamMember());
                })
                .toList();
    ...
    private void updateProjectLinks(Long projectId, List<ProjectLink> requestLinks) {
        projectLinkRepositoryPort.deleteAllByProjectId(projectId);
        projectLinkRepositoryPort.saveAll(requestLinks.stream()
