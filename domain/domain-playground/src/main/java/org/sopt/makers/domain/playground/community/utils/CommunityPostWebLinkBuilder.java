...
public class CommunityPostWebLinkBuilder {
  ...
  /**
   * categoryCode/parentCategoryCode는 storage 조회로 미리 해석된 canonical 코드다. (domain Category는 parentId만
   * 가지므로, 부모 코드 해석은 이 유틸의 책임 밖이다.)
   */
  public String build(
      Long postId, CommunityCategoryCode categoryCode, CommunityCategoryCode parentCategoryCode) {
    ...
    CommunityCategoryCode rootCategoryCode =
        parentCategoryCode != null ? parentCategoryCode : categoryCode;

    if (rootCategoryCode == CommunityCategoryCode.MEETING) {
      return baseUrl + "/group/post?id=" + postId;
    }

    StringBuilder webLink =
  ...
  private String resolveBaseUrl() {
    return Objects.equals(activeProfile, PROD_PROFILE) ? PROD_BASE_URL : DEV_BASE_URL;
  }

  private String resolveSubcategory(
      CommunityCategoryCode categoryCode, CommunityCategoryCode parentCategoryCode) {
    ...
    return code.startsWith(rootPrefix) ? code.substring(rootPrefix.length()) : code;
  }
}
