...
/**
 * 목록 조회 team 쿼리 파라미터의 canonical 값. 레거시 클라이언트는 한글 팀명("운영팀"/"미디어팀"/"임원진"/"메이커스")을 그대로 전달하므로 이를 우선
 * 인식하고, 영문 코드("EXECUTIVE" 등)도 하위 호환으로 계속 허용한다. 레거시와 동일하게 이 4개의 코드 외 값(빈 문자열, "해당 없음" 포함)은 필터 미적용으로
 * 취급한다.
 */
