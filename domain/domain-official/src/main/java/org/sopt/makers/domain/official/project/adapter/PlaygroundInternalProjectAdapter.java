...
import org.sopt.makers.domain.playground.project.port.OfficialProjectPort.OfficialProjectLinkInfo;
...
public class PlaygroundInternalProjectAdapter implements ProjectClientPort {
    ...
    public List<Project> fetchAll() {
        return officialProjectPort.fetchAll().stream()
                .map(this::toProject)
                .toList();
    }
    ...
    private ProjectDetail toProjectDetail(OfficialProjectDetailInfo info) {
        String projectImage = (info.images() != null && !info.images().isEmpty())
                ? info.images().get(0) : null;

        List<ProjectDetail.ProjectLink> links = info.links().stream()
                .map(l -> new ProjectDetail.ProjectLink(parseLinkType(l.linkTitle()), l.linkUrl()))
                .toList();

        List<ProjectMember> members = info.members().stream()
                .map(m -> new ProjectMember(m.name(), parseMemberRole(m.role()), m.description()))
                .toList();

        return new ProjectDetail(
                info.id(), info.name(), info.generation(),
                parseCategory(info.category()), parseServiceTypes(info.serviceType()),
                info.summary(), info.detail(), info.logoImage(), info.thumbnailImage(),
                projectImage, info.isFounding(), info.startAt(), info.endAt(),
