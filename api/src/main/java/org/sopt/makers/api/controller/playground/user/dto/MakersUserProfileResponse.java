...
public record MakersUserProfileResponse(
  ...
  public record MemberCareerResponse(
      Long id, String companyName, String title, Boolean isCurrent) {}

  public static MakersUserProfileResponse from(MakersUserProfile profile) {
    List<MemberSoptActivityResponse> activities =
        profile.activities().stream()
            .map(a -> new MemberSoptActivityResponse(a.id(), a.generation()))
            .toList();
    ...
    return new MakersUserProfileResponse(
        profile.id(), profile.name(), profile.profileImage(), activities, careers);
  }
}
