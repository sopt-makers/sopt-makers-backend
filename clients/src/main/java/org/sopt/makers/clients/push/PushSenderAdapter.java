...
public class PushSenderAdapter implements PushSenderPort {
  ...
  public PushSenderAdapter(AlarmProperty property) {
    this.client = new PushClient(property);
  }
  ...
  public void send(PushMessage message) {
    ...
    try {
      client.send(ACTION_SEND, PushSendRequest.from(message));
  ...
  private void manageToken(String action, PushToken pushToken) {
    try {
      client.manageToken(
          action, PushTokenManageRequest.from(pushToken), toPlatformHeader(pushToken.platform()));
    } catch (RestClientException e) {
