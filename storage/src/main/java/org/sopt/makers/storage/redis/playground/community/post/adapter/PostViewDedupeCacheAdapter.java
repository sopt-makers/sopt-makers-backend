 ...
 * 조회-후-저장 사이의 경쟁 상태 없이 원자적으로 "오늘 처음 조회했는지"를 판정한다. Redis 장애 시에는 중복방지를 포기하고 항상 조회수 증가를 허용한다(가용성 우선,
 * Top-50 캐시 등 다른 Redis 어댑터와 동일한 fail-open 정책).
 */
@Slf4j
@Component
...
public class PostViewDedupeCacheAdapter implements PostViewDedupePort {
  ...
  public boolean markAsViewedIfAbsent(Long userId, Long postId) {
    ...
    try {
      Boolean isFirstViewToday =
          stringRedisTemplate.opsForValue().setIfAbsent(key, VIEWED_MARK, TTL);
